...
namespace Ipfs.Core.Lib.BlockExchange
{
    ...
    public class Bitswap : IService
    {
        ...
        private readonly ConcurrentDictionary<Peer, BitswapLedger> peerLedgers =
            new ConcurrentDictionary<Peer, BitswapLedger>();

        private readonly ConcurrentDictionary<Cid, WantedBlock> wants = new ConcurrentDictionary<Cid, WantedBlock>();
        ...
        public Task StopAsync()
        {
            ...
            foreach (var cid in wants.Keys) Unwant(cid);
        ...
        /// </returns>
        /// <seealso cref="Ipfs.Abstractions.IBitswapApi.LedgerAsync(Peer, CancellationToken)" />
        public BitswapLedger PeerLedger(Peer peer)
        {
            if (peerLedgers.TryGetValue(peer, out var ledger)) return ledger;
...
#pragma warning restore VSTHRD100 // Avoid async void methods
        ...
        public Task<IDataBlock> WantAsync(Cid id, MultiHash peer, CancellationToken cancel)
        {
            ...
            var want = wants.AddOrUpdate(
                ...
                {
                    Id = id,
                    Consumers = new List<TaskCompletionSource<IDataBlock>>
                    {
                        tsc
                    },
                    Peers = new List<MultiHash>
                    {
                        peer
                    }
                },
            ...
            {
                var _ = SendWantListToAllAsync(new[]
                {
                    want
                }, false);
                BlockNeeded?.Invoke(this, new CidEventArgs
                {
                    Id = want.Id
                });
            }
        ...
        public void Unwant(Cid id)
        {
            ...
            if (wants.TryRemove(id, out var block))
        ...
        {
            if (wants.TryRemove(block.Id, out var want))
            {
