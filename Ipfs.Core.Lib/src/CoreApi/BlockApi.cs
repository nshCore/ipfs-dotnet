...
namespace TheDotNetLeague.Ipfs.Core.Lib.CoreApi
{
    ...
    internal class BlockApi : Abstractions.IBlockApi
    {
            ...
            CancellationToken cancel = default)
        {
            ...
            else
            {
                ...
                if (ipfs.IsStarted) await ipfs.Dht.ProvideAsync(cid, false, cancel).ConfigureAwait(false);
            ...
            // To pin or not.
            if (pin)
                await ipfs.Pin.AddAsync(cid, false, cancel).ConfigureAwait(false);
            else
                await ipfs.Pin.RemoveAsync(cid, false, cancel).ConfigureAwait(false);
        ...
        public async Task<Cid> RemoveAsync(Cid id, bool ignoreNonexistent = false, CancellationToken cancel = default)
        {
            ...
            if (await Store.ExistsAsync(id, cancel).ConfigureAwait(false))
            {
                ...
                await ipfs.Pin.RemoveAsync(id, false, cancel).ConfigureAwait(false);
                return id;
            }
